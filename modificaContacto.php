<?php
	...
	if(isset($_POST['guardar']))
	{
	    ...
	    $ultPago = $_POST['ultimoPago'];
	    $tipoPago = $_POST['type'];

	    if($tipoPago == 'Mensual')
		{
			$nuevafecha = strtotime ( '+1 month' , strtotime ( $ultPago ) ) ;
			$nuevafecha = date ( 'Y-m-j' , $nuevafecha );
		}
		else if($tipoPago == 'Trimestral')
		{
			$nuevafecha = strtotime ( '+3 month' , strtotime ( $ultPago ) ) ;
			$nuevafecha = date ( 'Y-m-j' , $nuevafecha );
		}
		else if($tipoPago == 'Anual')
		{
			$nuevafecha = strtotime ( '+1 year' , strtotime ( $ultPago ) ) ;
			$nuevafecha = date ( 'Y-m-j' , $nuevafecha );
		}
		...
		$resultado=$contacto->query("update  usuarios set nombre = '".$nombre."', apellido='".$apellidos."', codigo= '".$codigo."' , inscripcion= '".$fechains."' , ultimoPago='".$ultPago."', proximoPago ='".$nuevafecha."', tipoPago='".$tipoPago."', telefono='".$numero."', direccion='".$direccion."' where id='".$id."' ");
	}
	if($resultado>0)
	{echo '';
